...
<script>
	...
	function showAddStudentWin(){
		authority('com.oa.action.StudentAction.stuPurposeSave', "", "save",'');
	...
	function showEditStudentWin(){
		...
		if(rows.length  > 1 ){
			...
			if(row){
				...
				$("#editWin").window("open");
				authority('com.oa.action.StudentAction.stuPurposeUpdate', json1, "update",'');
			}
...
<%-- 
	...
	function disable(index){   // 禁用
		authority('com.oa.action.StudentAction.stuPurposeDisable', "", "disable",'');
	}
		
	function activate(index){  // 启用
		authority('com.oa.action.StudentAction.stuPurposeActivate', "", "activate",'');
	}
